...
##   cd build_mac && make src/experimental/honeycomb/freepressure.nim
##
## Arguments: -ntmax:32 -maxgrid:1024 -outdir:.
##
## Method
## ------
...
proc arg(name, default: string): string =
  ## value of a -name:value command-line argument
  for a in commandLineParams():
    if a.startsWith("-" & name & ":"): return a[name.len+2..^1]
  default
...
when isMainModule:
  let ntMax = parseInt(arg("ntmax", "32"))
  let maxGrid = parseInt(arg("maxgrid", "1024"))
  createDir(outDir)
  if ntMax < 32:
    stderr.writeLine "freepressure: ntMax must be at least 32 " &
      "(the fixed asymptotic fits require Nt=32)"
    quit(QuitFailure)
  if maxGrid < 48:
    stderr.writeLine "freepressure: maxGrid must be at least 48 " &
...
""")
  ...
  echo "wrote ", outDir / "pressure.dat", ", pressure_conv.dat, ",
       "pressure_series.dat, pressure.gnuplot (run gnuplot there for pressure.png)"
