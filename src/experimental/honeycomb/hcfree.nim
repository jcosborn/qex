...
##
## std/[math, complex] plus hcgeom only: 4x4 and 8x8 linear algebra in
## momentum space, no QEX layout or fields.
##
## a = 1, mu = 0..3 with mu = 3 the time direction, DeGrand-Rossi gammas
## as in QEX physics/spinOld (gamma1..gamma4 here gammaMat[0..3]).
##
## The operator
## ------------
...
##
## Brillouin zone: the site set Z^4 u (Z+1/2)^4 has reciprocal lattice
## 2 pi D4 (covolume 2 (2 pi)^4), so the BZ is twice the cubic one and an
## Ns^3 x Nt cell lattice carries 2 Ns^3 Nt momenta: the cubic set with one
## component's range doubled (`momenta` doubles p_0, `momentaAlt` p_3).
