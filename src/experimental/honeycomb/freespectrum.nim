...
##   cd build_mac && make src/experimental/honeycomb/freespectrum.nim
##
## Arguments: -ns:16 -nt:16 -bc:per|anti -outdir:.
## Default: 16^4, periodic in time (which is what makes the cubic spectrum
## reach exactly Re lambda = 8, as on the slide).
##
## Writes freespec_cubic.dat, freespec_16cell.dat and freespec.gnuplot to -outdir.
...
when isMainModule:
  let ns = parseInt(arg("ns", "16"))
  let nt = parseInt(arg("nt", "16"))
  let anti = arg("bc", "per").startsWith("a")
  createDir(outDir)
...
""")
  f.close()
  echo &"            -> {gp}"
  echo "            (run gnuplot there for freespec.png)"
