## refcubicgen -- quenched SU(3) configuration generation on the hypercubic
## lattice with the Wilson plaquette action (QEX gaugeAction1/gaugeForce,
## mdevolve integrators), the cubic reference for the honeycomb runs.
##
##   refcubicgen -lat:8,8,8,8 -beta:5.9 -ntraj:200 -nwarm:100 -outdir:/tmp/b590 -savefreq:2
##
## Output: <outdir>/cfg.NNNNN.lime, one HMC line per trajectory (dH,
## exp(-dH), accept, plaquette) and a SUMMARY line with the acceptance rate
## and <exp(-dH)>.
