suite "gauge fused":
  ...
  test "axpy finite differences":
    ckgradm3(axpy, x, gm, gu, y, 0.05*gq, gg, gp)

  test "adjmul finite differences":
    ckgradm2(adjmul, gg, gu, gp, gq, gm)

  test "muladj finite differences":
    ckgradm2(muladj, gg, gu, gp, gq, gm)

  test "contractProjTAH finite differences":
    ckgradm2(contractProjTAH, gg, gu, gp, gq, gm)

  test "axexp finite differences":
    ckgradm2(axexp, x, gm, y, 0.05*gq, gp)

  test "axexpmuly finite differences":
  ...
  test "fused gauge ops reject incompatible layouts at construction":
    ...
    expect(GraphValueError):
      discard blendSubset(-1, 0, gg, gu)
    expect(GraphValueError):
      discard blendSubset(0, g.len, gg, gu)
