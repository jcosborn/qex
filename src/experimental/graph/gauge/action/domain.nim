...
proc isFundamental*(gc: GaugeActionCoeffs): bool =
  ## Unshifted plaquette, rectangle, and parallelogram action.
  gc.adjplaq == 0
...
proc isGaugeAction*(gc: GaugeActionCoeffs): bool =
  gc.isFundamental or gc.isAdjPlaq
...
proc evalGaugeForceSubset*(gc: GaugeActionCoeffs, g, outg: types.Gauge, sd, sf, sb: auto, parity, dir: int) =
  ## Masked ambient derivative for the fundamental loop family.
  if not gc.isFundamental:
    raiseUnsupportedGaugeCoeff(gc)
...
proc evalGaugeForceJacobianSubset*(b: types.Gauge, gc: GaugeActionCoeffs, g, outg: types.Gauge, parity, dir: int) =
  ## outg = H_g(b[dir]|parity); writes all affected links.
  if not gc.isFundamental:
    raiseUnsupportedGaugeCoeff(gc)
