...
##
## See examples/generichmc.nim and prod/lsd/eightFlavorSMG.nim for examples of
...
proc add*(hmc: HmcAction; level: ActionLevel) = hmc.levels.insert(level)
...
proc generateReverse*(hmc: HmcAction) =
  let uc = hmc.uc
  let pp = addr hmc.p
  let lo = uc.u[0].l
  var g1 = lo.newGauge()
...
proc cold*(hmc: HmcAction) = hmc.uc.cold()
proc hot*(hmc: HmcAction) = hmc.uc.u.random(hmc.prng)
proc random*(hmc: HmcAction) = hmc.hot()
