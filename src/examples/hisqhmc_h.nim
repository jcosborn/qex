...
## 
## Author: Curtis Taylor Peterson, James Osborn
## 
## Details:
  ...
  defaultInputs = %* {
    ...
    "hmc": {
      ...
      "gauge-start": "cold" # start -- hot, cold, or read; 
                            # if read, need --ensemble flag (see newHISQHMC)
    },
    "action": {
    ...
    "fermion": {
      ...
      "steps": 10 # gauge steps per outer gauge update
    },
    "measurements": {
      "plaquette": {}, # tells isMainModule program to measure plaquette
      "polyakov": {}, # tells isMainModule program to measure Polyakov loop
    }
  }
...
proc randomComplexGaussian(phi: auto; pRNG: auto) =
  threads: phi.gaussian(pRNG)

proc randomComplexGaussian*(self: ParallelRNG; bosonField: auto) =
  case self.generator:
    of MILC: bosonField.gaussian(self.milc)
    of MRG: bosonField.gaussian(self.mrg)
