...
type
  ...
  QudaMemoryType* {.size: sizeof(cint).} = enum
    QUDA_MEMORY_INVALID = QUDA_INVALID_ENUM, QUDA_MEMORY_DEVICE = 0, QUDA_MEMORY_PINNED,
    QUDA_MEMORY_MAPPED
...
type
  ...
  QudaGaugeFieldOrder* {.size: sizeof(cint).} = enum
    QUDA_INVALID_GAUGE_ORDER = QUDA_INVALID_ENUM, QUDA_FLOAT_GAUGE_ORDER = 1, QUDA_FLOAT2_GAUGE_ORDER = 2, ##  no reconstruct and double precision
...
type
  ...
  QudaInverterType* {.size: sizeof(cint).} = enum
    ...
    QUDA_GCR_INVERTER, QUDA_MR_INVERTER, QUDA_MPBICGSTAB_INVERTER,
    QUDA_SD_INVERTER, QUDA_PCG_INVERTER, QUDA_MPCG_INVERTER, QUDA_EIGCG_INVERTER,
    ...
    QUDA_CA_CG_INVERTER, QUDA_CA_CGNE_INVERTER, QUDA_CA_CGNR_INVERTER,
...
type
  ...
  QudaMultigridCycleType* {.size: sizeof(cint).} = enum
    QUDA_MG_CYCLE_INVALID = QUDA_INVALID_ENUM, QUDA_MG_CYCLE_VCYCLE = 0,
    QUDA_MG_CYCLE_FCYCLE, QUDA_MG_CYCLE_WCYCLE, QUDA_MG_CYCLE_RECURSIVE
...
type
  ...
  QudaCloverFieldOrder* {.size: sizeof(cint).} = enum
    QUDA_INVALID_CLOVER_ORDER = QUDA_INVALID_ENUM, QUDA_FLOAT_CLOVER_ORDER = 1, ##  even-odd float ordering
  ...
  QudaVerbosity* {.size: sizeof(cint).} = enum
    QUDA_INVALID_VERBOSITY = QUDA_INVALID_ENUM, QUDA_SILENT = 0, QUDA_SUMMARIZE,
    QUDA_VERBOSE, QUDA_DEBUG_VERBOSE
  QudaTune* {.size: sizeof(cint).} = enum
    QUDA_TUNE_INVALID = QUDA_INVALID_ENUM, QUDA_TUNE_NO = 0, QUDA_TUNE_YES
...
type
  QudaDiracType* {.size: sizeof(cint).} = enum
    ...
    QUDA_CLOVER_DIRAC, QUDA_CLOVER_HASENBUSCH_TWIST_DIRAC,
    QUDA_CLOVER_HASENBUSCH_TWISTPC_DIRAC, QUDA_CLOVERPC_DIRAC,
    QUDA_DOMAIN_WALL_DIRAC, QUDA_DOMAIN_WALLPC_DIRAC, QUDA_DOMAIN_WALL_4D_DIRAC,
...
type
  QudaFieldOrder* {.size: sizeof(cint).} = enum
    QUDA_INVALID_FIELD_ORDER = QUDA_INVALID_ENUM, QUDA_FLOAT_FIELD_ORDER = 1, ##  spin-color-complex-space
  ...
  QudaFieldCreate* {.size: sizeof(cint).} = enum
    QUDA_INVALID_FIELD_CREATE = QUDA_INVALID_ENUM, QUDA_NULL_FIELD_CREATE = 0, ##  create new field
  ...
  QudaGammaBasis* {.size: sizeof(cint).} = enum
    QUDA_INVALID_GAMMA_BASIS = QUDA_INVALID_ENUM, QUDA_DEGRAND_ROSSI_GAMMA_BASIS = 0,
    QUDA_UKQCD_GAMMA_BASIS, QUDA_CHIRAL_GAMMA_BASIS
...
type
  ...
  QudaTwistFlavorType* {.size: sizeof(cint).} = enum
    QUDA_TWIST_INVALID = QUDA_INVALID_ENUM, QUDA_TWIST_DEG_DOUBLET = -2, QUDA_TWIST_NO = 0,
    QUDA_TWIST_SINGLET = 1, QUDA_TWIST_NONDEG_DOUBLET = +2
...
type
  ...
  QudaFieldGeometry* {.size: sizeof(cint).} = enum
    ...
    QUDA_VECTOR_GEOMETRY = 4, QUDA_TENSOR_GEOMETRY = 6, QUDA_COARSE_GEOMETRY = 8
  ...
  QudaStaggeredPhase* {.size: sizeof(cint).} = enum
    ...
    QUDA_STAGGERED_PHASE_MILC = 1, QUDA_STAGGERED_PHASE_CPS = 2,
    QUDA_STAGGERED_PHASE_TIFR = 3
  QudaContractType* {.size: sizeof(cint).} = enum
    QUDA_CONTRACT_TYPE_INVALID = QUDA_INVALID_ENUM, QUDA_CONTRACT_TYPE_OPEN = 0, ##  Open spin elementals
