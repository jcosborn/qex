...
proc init*(
    ...
    eps: float = 1e-16,
    maxiters: int = 10,
    delta: float = 1e-5 # arXiv:1004.0342
    # ^^^ only activated if unitaryProjectCutoff = true in smearGetForce
  ) =
  var f7lf = 0.0
  var naik = 1.0
  ...
  if c.projection.policy == CayleyHamilton: c.projection.delta = delta
