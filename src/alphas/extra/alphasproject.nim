...
import base
import cayleyhamilton
import maths/[types, complexNumbers, matrixFunctions]
...
proc newUnitaryProjection*(
  ...
  case policy:
    of CayleyHamilton: result.eps = eps
...
proc projectU*(self: UnitaryProjection; v: var Mat1; regulate: bool = false) =
  case self.policy:
    of CayleyHamilton: 
      if regulate: v.projectUCayleyHamilton(eps = self.delta) 
      else: v.projectUCayleyHamilton()
...
proc projectU*(self: UnitaryProjection, v: var Mat1; u: Mat2, regulate: bool = false) =
  ...
  v := u
  self.projectU(v, regulate = regulate)

template projectU*(
    self: UnitaryProjection, 
    r: var Color,
    regulate: bool = false
  ) = self.projectU(r[], regulate = regulate)

template projectU*(
    self: UnitaryProjection, 
    r: var Color, x: Color2,
    regulate: bool = false
  ) = self.projectU(r[], x[], regulate = regulate)
...
proc projectUderiv*(
  ...
  case self.policy:
    of CayleyHamilton: 
      if regulate: v.projectUderivCayleyHamilton(u, x, chain, eps = self.delta)
      else: v.projectUderivCayleyHamilton(u, x, chain)
    of GoloubKahanReinsch: discard
...
proc projectUderiv*(
  ...
  self.projectU(u, x)
  self.projectUderiv(v, u, x, chain, regulate = regulate)
...
template projectUderiv*(
    ...
    chain: Color4,
    regulate: bool = false
  ) = self.projectUderiv(r[], u[], x[], chain[], regulate = regulate)
...
template projectUderiv*(
    ...
    chain: Color4,
    regulate: bool = false
  ) = self.projectUderiv(r[], x[], chain[], regulate = regulate)
