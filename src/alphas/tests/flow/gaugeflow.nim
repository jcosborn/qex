...
# for regression against new code: ./bin/gaugeflow --flow-json=../../src/alphas/tests/flow/oldFlow.json --lattice-json=../../src/alphas/tests/flow/oldHMC.json

const 
  logStyle = "regress" #"KS_nHYP_FA"
  banner = """
|---------------------------------------------------------------|
...
for flow in flowInfo.keys(): 
  ...
  echo flowInfo[flow]["filename"].getStr()
