...
Running program:
   ...
   <parallel_exec> <location_of_bin_file> --start_traj=<first_trajectory> ...
   --end_traj=<last_trajectory> --save_freq=<frequency_of_data> ...
   --xml=<location_of_xml_file> --rank_geom=<layout_of_rank_geometry> ...
   --path=<input/output_path> --filename=<base_name_for_files>

   If "start_traj" is 0, then a new gauge configuration will be created
...
proc initialize_params_fields_and_rngs(): auto =
   ...
   while true:
      ...
      case cm_opts.kind
         ...
         of cmdShortOption, cmdLongOption, cmdArgument:
            ...
            if cm_opts.key == "start_traj":
               # Set ending trajectory
               start_traj = parseInt(cm_opts.val)

               # Print ending trajectory
               echo "start traj: " & cm_opts.val
            # Check if ending trajectory
            if cm_opts.key == "end_traj":
               # Set ending trajectory
               end_traj = parseInt(cm_opts.val)

               # Print ending trajectory
               echo "end traj: " & cm_opts.val
   ...
   let global_rng_file = io_path & def_fn & "." & intToStr(start_traj) & ".global_rng"   

   # Set name for RNG field field
   let field_rng_file = io_path & def_fn & "." & intToStr(start_traj) & ".rng"
   ...
   # Set gauge file name
   let gaugefile = io_path & def_fn & "." & intToStr(start_traj) & ".lat"
...
proc checkSolvers(rel_traj: int) =
   # Check if pbp to be measured
   if (int_prms["pbp_freq"] > 0) and (rel_traj mod int_prms["pbp_freq"] == 0):
...
for traj in start_traj..<end_traj:
   #[ Start HMC ]#
   ...
   echo "\n~~~~ Trajectory #: " & intToStr(traj)  & " ~~~~\n"

   # Define relative trajectory number from starting trajectory
   let rel_traj = traj + 1 - start_traj
   ...
   if (int_prms["rev_check_freq"] > 0) and (rel_traj mod int_prms["rev_check_freq"] == 0):
   ...
   if traj >= int_prms["no_metropolis_until"]:
      # Draw random number 
      accr = R.uniform
   ...
   if (save_freq > 0) and (rel_traj mod save_freq == 0):
      # Start timer
      tic("save")

      # Create filename
      let fn = io_path & def_fn & "." & intToStr(traj+1)
   ...
   if (int_prms["plaq_freq"] > 0) and (rel_traj mod int_prms["plaq_freq"] == 0):
      # Measure plaquette
      g.mplaq

   # Check if Polyakov loop to be measured
   if (int_prms["ploop_freq"] > 0) and (rel_traj mod int_prms["ploop_freq"] == 0):
      # Measure polyakov loop
      g.ploop
   ...
   if (int_prms["wflow_freq"] > 0) and (rel_traj mod int_prms["wflow_freq"] == 0):
   ...
   # Check solvers
   if (int_prms["check_solvers"] > 0) and (rel_traj mod int_prms["check_solvers"] == 0):
      # Check solvers
      checkSolvers(rel_traj)

   # Give user timing information
   tocc("\nTotal time for traj. #" & intToStr(traj) & ":", init_time)
