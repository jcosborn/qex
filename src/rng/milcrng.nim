...
type
  ...
  RngMilcV*[RMK: static RngMilcKind] = object
    ...
    when RMK == rmkFuel:
      ...
      iset: int32
      gset: float
  RngMilcV6* = RngMilcV[rmkV6]
  RngFuel* = RngMilcV[rmkFuel]
when defined(FUELCompat):
  type RngMilc6* = RngFuel
else:
  type RngMilc6* = RngMilcV6

template isWrapper*(x: RngMilcV): bool = false
template isWrapper*(x: typedesc[RngMilcV]): bool = false
template has*(x: typedesc[RngMilcV], y: typedesc): bool = y is RngMilcV
template numberType*(x: RngMilcV): typedesc = uint32
template numberType*(x: typedesc[RngMilcV]): typedesc = uint32
template simdLength*(x: typedesc[RngMilcV]): untyped = 1
template getNc*(x: RngMilcV): untyped = 0
template getNs*(x: RngMilcV): untyped = 0
template `:=`*(x: RngMilcV, y: RngMilcV) =
  x = y
template `:=`*(x: RngMilcV, y: Indexed) =
  x := y[]
template `[]`*(x: RngMilcV, y: Simd): untyped = x
template `[]=`*(x: RngMilcV, y: Simd, z: typed) =
  x := z

proc `$`*(x:RngMilcV6):string =
...
proc `$`*(x:RngFuel):string =
  result = "RngFuel r:[ " & $x.r0
  result &= " " & $x.r1
  result &= " " & $x.r2
  result &= " " & $x.r3
  result &= " " & $x.r4
...
template maxInt*(x: RngMilcV): int = int(MASK)
template maxInt*(x: typedesc[RngMilcV]): int = int(MASK)
template high*(x: RngMilcV): uint = uint(MASK)
template high*(x: typedesc[RngMilcV]): uint = uint(MASK)
template numInts*(x: RngMilcV): int = int(NUMINTS)
template numInts*(x: typedesc[RngMilcV]): int = int(NUMINTS)

# Try to strictly follow QLA_seed_random.c
proc seedX(prn: var RngFuel; seed0,index: int32) =
  const
    INDX1 = 69607'i32
    INDX2 = 8'i32
    ADDEND = 12345'i32
  ...
  prn.iset = 1
  prn.gset = 0
proc seedX(prn: var RngFuel; seed0,index: uint32) = seedX(prn, seed0.int32, index.int32)

proc seedX(prn: var RngMilcV6; seed0,index: uint32) =
  ## Seed the generator
  ## "index" selects which random number generator - which multiplier
  var seed = seed0
  template set(x: uint32) =
...
proc seedIndep*(prn: var RngMilcV; sed,index: auto) =
  seedX(prn, sed.uint32, index.uint32)
proc seed*(prn: var RngMilcV; sed,index: auto) =
...
func nextI(prn: var RngMilcV): uint32 {.inline.} =
...
func skip*(prn: var RngMilcV, c = 1) =
  for i in 1..c:
    discard prn.nextI

proc integer*(prn: var RngMilcV): int =
  ## Return random integer from 0 to maxInt
  result = int prn.nextI

proc next*(prn: var RngMilcV): uint =
  ## Return random integer from 0 to maxInt
...
proc gaussian*(prn: var RngFuel): float32 =
  ## Gaussian normal deviate
  ## Probability distribution exp( -x\*x/2 ), so < x^2 > = 1
  if prn.iset != 0:
    prn.iset = 0
...
template gaussian*(x: var auto, r: MaskedObj[RngMilcV]) =
  mixin gaussian
  gaussian(x, r[])
